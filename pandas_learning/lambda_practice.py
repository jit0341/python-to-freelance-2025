...
df1 = pd.DataFrame({'Age': [14,48,36,12,72]})
df1['Type'] = df1['Age'].apply(lambda x: 'Adult' if x >=18 else 'Child')
print("\nExample 1.3:")
print(df1)

# Price Classification 3
df2 = pd.DataFrame({'Price': [344,20,1100,9000,2000]})
df2['Category'] = df2['Price'].apply(lambda x: 'High' if x >=1000 else 'Low')
print("\nExample 2.3:")
print(df2)

# Score Classification 3
df3 = pd.DataFrame({'Score': [57,27,72,42,89]})
df3['Result'] = df3['Score'].apply(lambda x: 'Pass' if x >= 40 else 'Fail')
print("\nExample 3.3:")
print(df3)

# Stock Classification 3
df4 = pd.DataFrame({'Stock': [1,100,1000,0,50]})
df4['Status'] = df4['Stock'].apply(lambda x: 'Available' if x >0 else 'Out')
print("\nExample 4.3:")
print(df4)

# Temperature Classificatiom 3
df5 = pd.DataFrame({'Temp': [20,40,25,50,1]})
df5['Weather'] = df5['Temp'].apply(lambda x: 'Hot' if x >25 else 'Cold')
print("\nExample 5.3:")
print(df5)
